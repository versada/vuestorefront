...
class Website(models.Model):
    ...
    vsf_mailing_list_id = fields.Many2one(
        "mailing.list", "Newsletter", domain=[("is_public", "=", True)]
    )
    ...
    def _vsf_request_cache_invalidation(self):
        ...
        if url and key:
            try:
                ...
                _logger.error(
                    "Something went wrong processing cache invalidation", exc_info=True
                )
...
class WebsiteMenu(models.Model):
    ...
    is_footer = fields.Boolean(default=False)
    menu_image_ids = fields.One2many(
        "website.menu.image", "menu_id", string="Menu Images"
    )
