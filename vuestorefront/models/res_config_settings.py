...
class ResConfigSettings(models.TransientModel):
    ...
    vsf_payment_success_return_url = fields.Char(
        ...
        related="website_id.vsf_payment_success_return_url",
        readonly=False,
        required=True,
    )
    vsf_payment_error_return_url = fields.Char(
        "Payment Error Return Url",
        related="website_id.vsf_payment_error_return_url",
        readonly=False,
        required=True,
    )
    ...
    vsf_mailing_list_id = fields.Many2one(
        "mailing.list",
        "Newsletter",
        domain=[("is_public", "=", True)],
        related="website_id.vsf_mailing_list_id",
        readonly=False,
        required=True,
    )

    # VSF Images
    vsf_image_quality = fields.Integer("Vue Storefront Image Quality", required=True)
    vsf_image_background_rgba = fields.Char("Background RGBA", required=True)
    vsf_image_resize_limit = fields.Integer(
        "Resize Limit",
        required=True,
    ...
    def get_values(self):
        ...
        res.update(
            ...
            vsf_image_background_rgba=ICP.get_param(
                "vsf_image_background_rgba", "(255, 255, 255, 255)"
            ),
