...
class ProductTemplate(models.Model):
    ...
    def _auto_init(self):
        """Create/set slug via SQL to save on memory usage."""
        cr = self.env.cr
        if not column_exists(cr, "product_template", "slug"):
            create_column(cr, "product_template", "slug", "varchar")
            cr.execute("SELECT id, name FROM product_template WHERE name IS NOT NULL")
            for res in cr.fetchall():
                id_, name = res
                cr.execute(
                    "UPDATE product_template SET slug = %s WHERE id = %s",
    ...
    def _compute_slug(self):
        for product in self:
            ...
            if not isinstance(product.id, models.NewId):
                product.slug = slugify_with_sfx(product.name, f"-{product.id}")
