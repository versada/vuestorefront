...
class Http(models.AbstractModel):
    ...
    def _content_image_get_response(
        ...
        Center image in background with color, resize, compress and convert
        image to webp or jpeg.
        """
        if status == 200 and image_base64 and width and height:
            try:
                ...
                try:
                    background_rgba = safe_eval(
                        ICP.get_param(
                            "vsf_image_background_rgba", "(255, 255, 255, 255)"
                        )
                    )
                # FIXME: this should handle only specific exceptions, not
                # all.
                except Exception:
        ...
        return super()._content_image_get_response(
            status,
            headers,
            image_base64,
            model=model,
