...
def predicate_maillist_id(maillist_id):
    return lambda mail: mail.list_id.id == maillist_id
...
class MailingListQuery(graphene.ObjectType):
    ...
    def resolve_mailing_list(self, info, id):
        return (
            info.context["env"]["mailing.list"]
            .sudo()
            .search([("id", "=", id), ("is_public", "=", True)], limit=1)
        )

    @staticmethod
    def resolve_mailing_lists(
        self, info, filter, current_page, page_size, search, sort
    ):
        env = info.context["env"]
        order = get_search_order(sort)
...
class UserAddMultipleMailing(graphene.Mutation):
    ...
    def mutate(self, info, mailings):
        ...
        mailing_contact = (
            env["mailing.contact"].sudo().search([("email", "=", user.email)], limit=1)
        )
        ...
        for mailing in mailings:
            ...
            mailing_list = (
                env["mailing.list"].sudo().search([("id", "=", maillist_id)], limit=1)
            )
            if not mailing_list:
                raise GraphQLError(_("Maillist does not exist."))

            if mailing_contact:
                line = mailing_contact.subscription_list_ids.filtered(
                    predicate_maillist_id(maillist_id)
                )

                if not mailing_contact.company_name or (
                    company_name and mailing_contact.company_name != company_name
                ):
                    mailing_contact.update({"company_name": company_name})

                if not mailing_contact.country_id or (
                    country_id and mailing_contact.country_id != country_id
                ):
                    ...
                    mailing_contact.write(
                        {
                            "subscription_list_ids": [
                                (0, 0, {"list_id": mailing_list.id, "opt_out": optout})
                            ],
                        }
                    )
            else:
