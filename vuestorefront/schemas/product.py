...
from .objects import Attribute, AttributeValue, Product, SortEnum
...
def get_search_domain(env, search, **kwargs):
    ...
    if kwargs.get("category_slug", False):
        domains.append(
            [("public_categ_slug_ids.website_slug", "=", kwargs["category_slug"])]
        )
...
def get_product_list(env, current_page, page_size, search, sort, **kwargs):
    ...
    # If attribute values are selected, we need to get the full list of
    # attribute values and prices
    if domain == partial_domain:
        ...
        attribute_values = without_attributes_products.mapped(
