...
from .objects import PaymentProvider, PaymentTransaction
from .shop import Cart, CartData
        ...
        domain = expression.AND(
            [
                ...
                [
                    ...
                    ("available_country_ids", "in", [order.partner_id.country_id.id]),
                ],
            ]
        )
...
class PaymentMutation(graphene.ObjectType):
    make_gift_card_payment = MakeGiftCardPayment.Field(
        description="Pay the order only with gift card."
    )
...
class AdyenTransaction(graphene.Mutation):
    ...
    def mutate(self, info, provider_id):
        ...
        transaction_id = PaymentTransaction.search(
            [("reference", "=", transaction["reference"])], limit=1
        )
...
class AdyenPayments(graphene.Mutation):
    class Arguments:
        ...
        payment_method = generic.GenericScalar(
            required=True, description="Return state.data.paymentMethod"
        )
        browser_info = generic.GenericScalar(
            required=True, description="Return state.data.browserInfo"
        )
    ...
    def mutate(
        ...
        converted_amount = payment_utils.to_minor_currency_units(
            ...
            arbitrary_decimal_number=CURRENCY_DECIMALS.get(
                transaction.currency_id.name, 2
            ),
        )
...
class AdyenPaymentDetails(graphene.Mutation):
    class Arguments:
        provider_id = graphene.Int(required=True)
        transaction_reference = graphene.String(required=True)
        payment_details = generic.GenericScalar(
            required=True, description="Return state.data"
        )
...
class AdyenPaymentMutation(graphene.ObjectType):
    adyen_provider_info = AdyenProviderInfo.Field(
        description="Get Adyen Provider Info."
    )
    adyen_payment_methods = AdyenPaymentMethods.Field(
        description="Get Adyen Payment Methods."
    )
    ...
    adyen_payment_details = AdyenPaymentDetails.Field(
        description="Submit the Adyen Payment Details."
    )
