...
class VSFBinary(Binary):
    @http.route(
        [
            ...
            (
                "/web/image/<string:model>/<int:id>/"
                + "<string:field>/<int:width>x<int:height>/<string:filename>"
            ),
            "/web/image/<int:id>",
...
class GraphQLController(http.Controller, GraphQLControllerMixin):
    def _process_request(self, schema, data):
        ...
        return super()._process_request(schema, data)
    ...
    def _set_website_context(self):
        ...
        try:
            ...
            website = request.env["website"].search(
                [("domain", "ilike", request_host)], limit=1
            )
            if website:
                ...
                if request_uid != website_uid and request.env[
                    "res.users"
                ].sudo().browse(request_uid).has_group("base.group_public"):
