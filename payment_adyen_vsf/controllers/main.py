...
class AdyenControllerInherit(AdyenController):
    ...
    def adyen_payments(
        ...
        :param int provider_id: The provider handling the transaction, as
             `payment.provider` id
        :param str reference: The reference of the transaction
        :param int converted_amount: The amount of the transaction in minor
            units of the currency
        :param int currency_id: The currency of the transaction, as a `res.currency` id
        :param int partner_id: The partner making the transaction, as a `res.partner` id
        :param dict payment_method: The details of the payment method used for
            the transaction
        ...
        :rtype: dict
        """
        # Check that the transaction details have not been altered. This allows
        # preventing users
        # from validating transactions by paying less than agreed upon.
        if not payment_utils.check_access_token(
            access_token, reference, converted_amount, partner_id
        ):
            raise ValidationError(_("Adyen: Received tampered payment request data."))
        ...
        if tx_sudo.created_on_vsf:
            if (
                request.httprequest.headers.environ.get("HTTP_REAL_IP", False)
                and request.httprequest.headers.environ["HTTP_REAL_IP"]
            ):
        ...
        data = {
            ...
            # Most susceptible to trigger a 3DS check
            ...
            "returnUrl": urls.url_join(
                ...
                # Include the reference in the return url to be able to match it
                # after redirection.
                # The key 'merchantReference' is chosen on purpose to be the
                # same as that returned
        ...
        # Force the capture delay on Adyen side if the provider is not configured
        # for capturing
        # payments manually. This is necessary because it's not possible to distinguish
        # 'AUTHORISATION' events sent by Adyen with the merchant account's capture
        # delay set to
        # 'manual' from events with the capture delay set to 'immediate' or a number
        # of hours. If
        # the merchant account is configured to capture payments with a delay but
        # the provider is
        # not, we force the immediate capture to avoid considering authorized
        #  transactions as
        # captured on Odoo.
    ...
    def adyen_return_from_3ds_auth(self, **data):
        """Process the authentication data sent by Adyen.

        Its done after redirection from the 3DS1 page.

        The route is flagged with `save_session=False` to prevent Odoo
        from assigning a new session
        to the user if they are redirected to this route with a POST
        request. Indeed, as the session
        cookie is created without a `SameSite` attribute, some browsers
        that don't implement the
        recommended default `SameSite=Lax` behavior will not include the
        cookie in the redirection
        request from the payment provider to Odoo. As the redirection to
        the '/payment/status' page
        will satisfy any specification of the `SameSite` attribute, the
        session of the user will be
        retrieved and with it the transaction which will be immediately
        post-processed.

        :param dict data: The authentication result data. May include custom
            params sent to Adyen in the request to allow matching the
            transaction when redirected here.
        """
        payment_transaction = data.get("merchantReference") and request.env[
            "payment.transaction"
        ].sudo().search([("reference", "in", [data.get("merchantReference")])], limit=1)
        ...
        sale_order = (
            ...
            .sudo()
            .search(
                [("id", "in", sale_order_ids), ("website_id", "!=", False)], limit=1
            )
        )
        ...
        # Overwrite the operation to force the flow to 'redirect'. This is
        # necessary because even
        # thought Adyen is implemented as a direct payment provider, it will
        # redirect the user out
        # of Odoo in some cases. For instance, when a 3DS1 authentication is
        # required, or for
        # special payment methods that are not handled by the drop-in (e.g. Sofort).
        tx_sudo.operation = "online_redirect"

        # Query and process the result of the additional actions that have been
        # performed
        _logger.info(
            "handling redirection from Adyen for transaction with reference"
            + " %s with data:\n%s",
            tx_sudo.reference,
            pprint.pformat(data),
        )
        ...
        if payment_transaction.created_on_vsf:
            ...
            if (
                result
                and result.get("resultCode")
                and result["resultCode"] == "Authorised"
            ):
            ...
            # For Redirect 3DS2 and MobilePay (Cancel/Error flow)
                ...
                result
                and result.get("resultCode")
                and result["resultCode"] in ["Refused", "Cancelled"]
            ):
    ...
    def adyen_webhook(self):
        ...
        for the exhaustive list of event codes.
        ...
        for notification_item in data["notificationItems"]:
            ...
            try:
                payment_transaction = notification_data.get(
                    "merchantReference"
                ) and PaymentTransaction.sudo().search(
                    [("reference", "in", [notification_data.get("merchantReference")])],
                    limit=1,
                )
                ...
                # Handle the notification data as if they were feedback of a
                # S2S payment request
                ...
                if (
                    ...
                    and success
                    and payment_transaction.created_on_vsf
                ):
                    # Check the Order and respective website related with the
                    # transaction Check the payment_return url for the success
                    # and error pages
                    sale_order_ids = payment_transaction.sale_order_ids.ids
                    sale_order = (
                        request.env["sale.order"]
                    ...
                    website = sale_order.website_id
                    # Redirect to VSF
                    vsf_payment_success_return_url = (
                        website.vsf_payment_success_return_url
                    )
